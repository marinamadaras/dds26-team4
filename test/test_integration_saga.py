...
- stock quantity is actually reduced
- user credit is actually deducted
...
class TestIntegrationCheckout:
    ...
    def test_stock_is_reduced_after_checkout(self):
        ...
        # Act
        checkout(order_id)
        status = wait_for_order_status(order_id, "COMPLETED")

        # Assert
        assert status == "COMPLETED", f"Order did not complete, status={status}"
    ...
    def test_insufficient_credit_cancels_order_and_keeps_stock(self):
        """If user has insufficient credit, order is cancelled and stock is NOT reduced."""
        # Arrange
        ...
        assert get_item_stock(item_id) == 5   # stock must be restored
        assert get_user_credit(user_id) == 10  # credit unchanged

    def test_insufficient_stock_cancels_order_and_keeps_credit(self):
        """If item has insufficient stock, order is cancelled and credit is NOT deducted."""
        # Arrange
        ...
        # Assert
        assert status == "CANCELLED", f"Expected CANCELLED, got={status}"
        assert get_user_credit(user_id) == 500  # credit must be untouched
